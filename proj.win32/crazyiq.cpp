    ...
    int       nCmdShow)
{
    ...
    return CrazyiqPlayer::createAndRun();
}

int CrazyiqPlayer::createAndRun(void)
{
    CrazyiqPlayer *host = CrazyiqPlayer::sharedInstance();
    int ret = host->run();
    CrazyiqPlayer::purgeSharedInstance();
    return ret;
}

CrazyiqPlayer *CrazyiqPlayer::s_sharedInstance = NULL;

CrazyiqPlayer *CrazyiqPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new CrazyiqPlayer();
    }
    return s_sharedInstance;
}

void CrazyiqPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
CrazyiqPlayer::CrazyiqPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int CrazyiqPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "crazyiq-player");
...
void CrazyiqPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void CrazyiqPlayer::createViewMenu(void)
{
...
void CrazyiqPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void CrazyiqPlayer::updateWindowTitle(void)
{
}

void CrazyiqPlayer::relaunch(void)
{
...
void CrazyiqPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void CrazyiqPlayer::onFileOpenProject(void)
{
...
void CrazyiqPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"CrazyiqPlayer");
...
void CrazyiqPlayer::onFileProjectConfig(void)
{
...
void CrazyiqPlayer::onFileRelaunch(void)
{
    relaunch();
}

void CrazyiqPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void CrazyiqPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void CrazyiqPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void CrazyiqPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void CrazyiqPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT CrazyiqPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    CrazyiqPlayer *host = CrazyiqPlayer::sharedInstance();
...
INT_PTR CALLBACK CrazyiqPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
